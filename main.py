...
def addDownloadItem(url):
    if url!=None:
        ...
        fname.replace(" ","")
        frame2=Frame(frame,bg="#787878")
        img=Image.open("file_icon.png")
        render=ImageTk.PhotoImage(img)

        label=Label(frame2,image=render,bg="#787878")
        label.image=render
        label.grid(row=0,column=0,rowspan=2)

        title=Label(frame2,text=fname,padx=5,bg="#787878",fg="white",anchor="w")
        ...
        labelPercentage=Label(frame2,text="0 %",padx="5",anchor="w",bg="#787878",fg="white")
        labelPercentage.grid(row=0,column=2)
        labelsize=Label(frame2,text="0 KB",padx="5",anchor="w",bg="#787878",fg="white")
...
rowframe=Frame(frame,bg="#000000")
button=Button(rowframe,text="Add Download URL",bg="#2740ae",fg="white",padx=5,pady=5,activebackground="#2740ae",activeforeground="white",command=PassUrlData)
